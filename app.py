...
MSG_BOAS_VINDAS = (
    ...
    "Após pagar, *envie o comprovante aqui no chat* para que o Henrique libere seu acesso agora! 🔥"
)
...
def obter_info_aluno(jid):
    try:
        ...
        return False, -1 # -1 significa que não existe no banco
    except:
        return False, -1
...
def webhook():
    ...
    if dados.get("event") == "messages.upsert":
        try:
            ...
            # --- COMANDOS DE ADMIN (Só funcionam se VOCÊ enviar) ---
            if from_me:
                # COMANDO #LIBERAR
                if texto.lower() == "#liberar":
                    ...
                    requests.post(f"{EVO_URL}/message/sendText/{INSTANCIA}", 
                                  json={"number": numero_jid, "text": "✅ *ACESSO VIP LIBERADO!*"}, headers={"apikey": EVO_KEY})
                    return "OK", 200

                # COMANDO #REMOVER (Para você resetar o aluno e testar o fluxo)
                if texto.lower() == "#remover":
                    ...
                    conn.close()
                    requests.post(f"{EVO_URL}/message/sendText/{INSTANCIA}", 
                                  json={"number": numero_jid, "text": "🗑️ *USUÁRIO REMOVIDO DO BANCO!* Próximo 'Oi' será como novo aluno."}, headers={"apikey": EVO_KEY})
                    return "OK", 200
                
                return "OK", 200 # Ignora outras mensagens suas

            # --- FLUXO DO ALUNO ---
            pago, dias = obter_info_aluno(numero_jid)
            
            # Bloqueia se não pagou, se venceu ou se não existe no banco
            if not pago or dias < 0:
                requests.post(f"{EVO_URL}/message/sendText/{INSTANCIA}", 
                              json={"number": numero_jid, "text": MSG_BOAS_VINDAS}, headers={"apikey": EVO_KEY})
                return "OK", 200

            # --- RESPOSTA DA IA (SÓ PARA VIPS) ---
            aviso_vencimento = ""
            if 0 <= dias <= 5:
                aviso_vencimento = f" (IMPORTANTE: Faltam {dias} dias de assinatura. Avise para renovar)."

            prompt_sistema = (
                "Você é o Coach Max, assistente fitness. "
                f"O aluno tem {dias} dias de acesso. "
                "Responda normalmente às dúvidas. NÃO mencione o tempo de assinatura, "
                "A MENOS que o aluno pergunte 'quanto tempo eu tenho' ou algo similar, "
                f"ou se este aviso estiver ativo: {aviso_vencimento}."
            )
